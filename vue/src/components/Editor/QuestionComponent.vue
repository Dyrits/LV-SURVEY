...
const types = computed(() => store.state.questions.types);
...
function removeOption(option) {
  setOptions(getOptions().filter(($option) => option !== $option));
  refresh();
}

function editType() {
  hasOptions() && setOptions(getOptions() || []);
  refresh();
}
